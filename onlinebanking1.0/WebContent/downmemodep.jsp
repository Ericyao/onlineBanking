...
<body class="modern-ui" onresize="Resize()">
 ...
 %>
							<span class="first-name"><%=Vfusername %></span> <span
								class="last-name">logout safety</span>
						</div>
				...
				<div style="position: absolute; left: 40px; bottom: -25px;">
					<a href="metro.jsp"><img src="images/back.png" width="70"
						height="70" alt="" /></a>
				</div>
			</div>
		</div>
		<%	String from =(String) request.getAttribute("transfernum"); 
				String amount = (String)request.getAttribute("Amout");
				String time = (String)request.getAttribute("TDmemotime");
				System.out.println("from+amount+time"+from+amount+time);
				
		    	request.setAttribute("fffrom",from);
		    	request.setAttribute("ftime",time);
		    	request.setAttribute("famount",amount); 
			%>
		<form action="SDownloadMemo.jsp?fffrom=${fffrom }&ftime=${ftime}&famount=${famount}" method="POST">
			<div>


				<table class="bordered"
					style="width: 10cm; margin-left: 5cm; margin-top: 5cm;">
					<thead>
					...
					<tr>
						<td>memo no this is timedeposit memo</td>
			...
			<input class="btn" type="submit" value="Download Memo">
		</form>
	</div>
</body>
</html>